...
--def Grammar [Fintype α] : Type := PEG (Fintype.card α)
    ...
    | mk le2_f le2_0 le2_s =>
      {
        ...
        }
        {
          match (Maybe.le.equiv_to_imply.mp le1_0) with
          | Or.inl h => simp [h]; match (Maybe.le.equiv_to_imply.mp le1_s) with
            | Or.inl h => simp only [h, Prod.mk.injEq, and_imp]; apply Maybe.le.lhs_unknown
            | Or.inr ⟨x',⟨y',⟨hx', hy'⟩⟩⟩ => simp [hx', hy']; match (Maybe.le.equiv_to_imply.mp le2_0) with
              | Or.inl h => simp only [h, Prod.mk.injEq, true_and, and_imp]; match (Maybe.le.equiv_to_imply.mp le2_s) with
                | Or.inl h => simp only [h, Prod.mk.injEq, true_and, and_imp]; apply Maybe.le.lhs_unknown
                | Or.inr ⟨x',⟨y',⟨hx', hy'⟩⟩⟩ => simp only [hx', hy', Prod.mk.injEq, true_and, and_imp]; cases (g_props e1 Q).snd.fst <;> cases (g_props e2 Q).snd.fst <;> simp <;> apply Maybe.le.all_found
              | Or.inr ⟨x',⟨y',⟨hx', hy'⟩⟩⟩ => simp only [hx', g_props.match_7.eq_1, hy']; apply Maybe.le.all_found
          | Or.inr ⟨x',⟨y',⟨hx', hy'⟩⟩⟩ => simp only [hx', Prod.mk.injEq, and_imp, hy']; match (Maybe.le.equiv_to_imply.mp le1_s) with
            | Or.inl h => simp only [h, Prod.mk.injEq, true_and, and_imp]; match (Maybe.le.equiv_to_imply.mp le2_0) with
              | Or.inl h => simp only [h, Prod.mk.injEq, true_and, and_imp]; match (Maybe.le.equiv_to_imply.mp le2_s) with
                | Or.inl h => simp only [h, Prod.mk.injEq, and_imp]; apply Maybe.le.lhs_unknown
                | Or.inr ⟨x',⟨y',⟨hx', hy'⟩⟩⟩ => simp only [hx', hy', Prod.mk.injEq, and_imp]; cases (g_props e1 Q).snd.snd <;> cases (g_props e2 Q).snd.fst <;> simp <;> apply Maybe.le.all_found
              | Or.inr ⟨x',⟨y',⟨hx', hy'⟩⟩⟩ =>  simp only [hx', hy', Prod.mk.injEq, and_imp]; match (Maybe.le.equiv_to_imply.mp le2_s) with
                | Or.inl h => simp only [h, Prod.mk.injEq, true_and, and_imp]; apply Maybe.le.lhs_unknown
                | Or.inr ⟨x',⟨y',⟨hx', hy'⟩⟩⟩ => simp only [hx', hy', forall_const, Prod.mk.injEq, true_and, and_imp]; cases (g_props e1 Q).snd.snd <;> simp <;> apply Maybe.le.all_found
            | Or.inr ⟨x',⟨y',⟨hx', hy'⟩⟩⟩ => simp only [hx', Prod.mk.injEq, true_and, and_imp, hy']; match (Maybe.le.equiv_to_imply.mp le2_0) with
              | Or.inl h => simp only [h, Prod.mk.injEq, true_and, and_imp]; match (Maybe.le.equiv_to_imply.mp le2_s) with
                | Or.inl h => simp only [h, Prod.mk.injEq, true_and, and_imp]; apply Maybe.le.lhs_unknown
                | Or.inr ⟨x',⟨y',⟨hx', hy'⟩⟩⟩ => simp only [hx', hy', forall_const, Prod.mk.injEq, true_and, and_imp]; cases (g_props e2 Q).snd.fst <;> simp <;> apply Maybe.le.all_found
              | Or.inr ⟨x',⟨y',⟨hx', hy'⟩⟩⟩ => simp only [hx', g_props.match_7.eq_1, hy']; apply Maybe.le.all_found
        }
      }
